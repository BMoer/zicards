...
- **07.09. — 2 unpushed Doku-Commits stehen weiterhin an** (`268b4b0`,
  ...
  nicht zu deployen (Push auf `main` löst den Vercel-Deploy aus). `git
  status --short` sonst sauber, `app`/`app-vercel`/`supabase` weiterhin
  200/200/200.
...
- **31.08. — zehnter Reminder-Tag in Folge, kein drittes Konto zurück, aber
  ...
  ununterbrochener Erfolgstag, 22.–31.08.). Frisch gemessen in
  `user_progress`/`sentence_progress`: **Rang-2-Account (`055164cb`)** hat
  zwischen 03:16 und 03:39 UTC heute Nacht eine neue Session gemacht (14 neue
  `user_progress`- + 15 `sentence_progress`-Zeilen) — der Account bleibt aktiv,
  aber es ist kein neuer (dritter) Rückkehrer. `bfe8c879` (Rückkehrer vom
  29.08.) bleibt unverändert bei seiner letzten Session vom 29.08.
  20:06–20:11 UTC stehen. Weiterhin exakt **2 von 13** Accounts seit der
  Reaktivierung (22.08.) aktiv — unverändert zum 30.08. Feedback unverändert
  0 offen von 41 gesamt (Content-Range-Header). Tests 167/167, Lint 0
  Fehler/11 unveränderte Warnungen, `npm audit` 0, `npx knip` identisch zum
  17.08.-Stand (5 unused files/1 unused dep/9 unused exports) — keine
  Regression. App/Supabase weiter 200/200/200. 1 unpushed Doku-Commit vor
  dieser Session (`c150acf`, 30.08.) plus eine bereits vorher uncommittete,
  fremde Änderung in diesem File (Satz-Kürzung im
  „Aus dem globalen Check-in"-Abschnitt vom 30.08., nicht von dieser
  Session — unangetastet gelassen, wird mit committet). **Todo
  `zicards/lerngruppe-nachfassen` wurde am 30.08. bereits von Ben
  geschlossen** — keine neue Prüfung des Postfachs, kein neuer Entwurf,
  Thema nicht unter neuem Key wieder aufgemacht.
...
- [ ] **Weiter beobachten: Rückkehr-Rate nach der Cron-Reaktivierung.** 07.09.
      neu gemessen: sechzehnter erfolgreicher Reminder-Tag bestätigt
      (22.08.–06.09., `last_reminder_sent` bei allen 4 reminder-fähigen
      `user_settings`-Zeilen auf 06.09. 07:00 UTC), der siebzehnte ist um
      Check-in-Zeit (07:2x CEST) noch nicht fällig (Job läuft 09:00 CEST) —
      geplanter Zustand, kein Ausfall. Aktive-Accounts-Zahl diese Session
      nicht neu erhoben (Profil verlangt sie nicht explizit) — laut
      Handover-Stand weiterhin 2 von 13. **Todo `zicards/lerngruppe-nachfassen`
      wurde am 30.08. von Ben geschlossen** (abgehakt = Prioritätsentscheidung,
      nicht Vollzugsmeldung) — dieser Punkt bleibt als reine Beobachtung
      stehen, ohne erneute Handlungsaufforderung an Ben.
- [ ] Fehlt Error-Tracking (Sentry o.ä.)? Unverändert — Ben entscheiden
      lassen, ob der Aufwand lohnt. **04.09.: bekommt zusätzliches Gewicht
      durch Bens Griechenland-Urlaub 22.–29.09.** — der Reminder-Cron ist
      der einzige laufende Automatismus dieses Projekts, und die einzige
      Beobachtung dafür ist bislang der tägliche Check-in selbst. Fällt der
      Cron während des Urlaubs aus, bekommen Nutzer schlicht keine
      Erinnerungsmail mehr — ohne Fehlermeldung, ohne dass es jemand
      bemerkt, bis der Check-in wieder läuft. Kein neuer Code gebaut (wäre
      ein Feature-Entscheid, Topf B), nur als Grund für die Sentry-Frage
      vermerkt.
...
- [ ] pi-lens-Cache insgesamt erneuern (Stand 13./14.04., jetzt ~4,5 Monate
      alt) — gleiche Ursache wie oben.
- [ ] **Vault-Seite `zicards` 5 Tage hinter dem Handover.** Letzter Compile
      04.09. laut Vault-Metadaten auf 2026-08-30 — die Fakten aus den
      Sessions 31.08./01.–03.09. (11./12./13. Erfolgstag, weiterhin 0/41
      Feedback, `VITE_COURSE_CODE` jetzt 144 statt „kann aufgeräumt werden")
      fehlen dort noch. Zusätzlich ein Doku-vs-Code-Fund: die Architecture-
      Tabelle der Vault-Seite nennt „158 Vitest, 13 test files", tatsächlich
      sind es **167 Tests, 13 Dateien** (`npm test` heute). Kein Schreibzugriff
...
- **2026-09-07** — Projekt-Check-in, Teil eines globalen Laufs (Bens
  ...
  Termin). Health 200/200/200 (App × 2, Supabase). Tests 167/167, Lint 0
  Fehler/11 unveränderte Warnungen, `npm audit` 0 Vulnerabilities. Feedback
  weiter 0 offen von 41 gesamt. **Reminder-Cron: sechzehnter Erfolgstag
  bestätigt** (22.08.–06.09., `last_reminder_sent` bei allen 4
  reminder-fähigen `user_settings`-Zeilen auf 06.09. 07:00 UTC), siebzehnter
  zur Check-in-Zeit (07:2x CEST) noch nicht fällig — geplanter Zustand, kein
  Ausfall. Todo `zicards/vite-course-code-entfernen` geprüft: bleibt offen,
  Prod-Env-Eingriff bewusst nicht ausgeführt (Topf B) — Befehl bereit
  (`vercel env rm VITE_COURSE_CODE production`), Variable jetzt 147 Tage alt,
  weiterhin 0 Code-Referenzen. **2 unpushed Doku-Commits vom 04.09. weiterhin
  nicht gepusht** (dieser Lauf ist angewiesen, nicht zu deployen). pi-lens-
  Caches unverändert (Stand 13./14.04., weiterhin extern blockiert). **Topf
  A:** keine offene HANDOVER-Position ohne Ben/Prod-Zugriff umsetzbar
  (anon-Grant-Entscheidung, `VITE_COURSE_CODE`-Entfernung und
  Error-Tracking bleiben Ben-Entscheidungen) — keine Code-Änderung diese
  Session, nur Messungen aktualisiert. Kein Deploy diese Session (kein
  Push).
- **2026-09-04** — Projekt-Check-in (Bens Fokus heute: halber Arbeitstag, ab
  14:30 Anreise/Hochzeiten bis So — kein Bezug zu zicards, keiner
  konstruiert). Health erneut 200/200/200 (App × 2, Supabase), `git status`
  sauber, HEAD = origin/main. Tests 167/167, Lint 0 Fehler/11 unveränderte
  Warnungen, `npm audit` 0 Vulnerabilities (mehrere Minor-Updates verfügbar,
  keine sicherheitsrelevant, keine Aktion). Feedback weiter 0 offen von 41
  gesamt. **Reminder-Cron: dreizehnter Erfolgstag bestätigt** (22.08.–03.09.,
  `last_reminder_sent` bei 4/5 `user_settings`-Zeilen auf 03.09. 07:00 UTC),
  vierzehnter zur Check-in-Zeit noch nicht fällig — geplanter Zustand, kein
  Ausfall. Todo `zicards/vite-course-code-entfernen` erneut geprüft, offen
  bestätigt (`vercel env ls production`, jetzt 144 Tage, weiterhin 0
  Code-Referenzen). **Zusätzliche Nachprüfungen dieser Session (Topf A):**
  keine toten Dateiverweise im Handover (Stichprobe über Backtick-Dateinamen,
  alle referenzierten Skripte/SQL-Dateien vorhanden); `.github/workflows/`
  bestätigt weiterhin nicht vorhanden; pi-lens-Caches unverändert (Stand
  13./14.04., weiterhin extern blockiert) — keine Regression. **Fund:**
  Vault-Seite `zicards` fünf Tage hinter dem Handover (letzter Compile
  30.08.) plus ein Doku-vs-Code-Mismatch (Testzahl 158 vs. tatsächlich 167) —
  siehe „Offene Punkte", Vorschlag für den nächsten globalen Vault-Ingest,
  hier nicht geschrieben (Projektmodus fasst den Vault nicht an). Bens
  Griechenland-Urlaub 22.–29.09. als Grund vermerkt, die offene
  Sentry/Error-Tracking-Frage nicht neu zu entscheiden, aber nicht weiter
  liegen zu lassen. **Topf A:** keine offene HANDOVER-Position ohne
  Ben/Prod-Zugriff umsetzbar (anon-Grant-Entscheidung,
  `VITE_COURSE_CODE`-Entfernung und Error-Tracking bleiben
  Ben-Entscheidungen; pi-lens-Cache-Reparatur bleibt extern blockiert) —
  keine Code-Änderung diese Session, nur Messungen aktualisiert und
  Nachprüfungen ergänzt. Kein Deploy diese Session (kein Push).
